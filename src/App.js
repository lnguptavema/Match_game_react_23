...
const rand = NewList[Math.ceil(Math.random() * 30)]
...
class App extends Component {
  ...
  filteredList = items => {
    ...
    const newList = items.filter(eachItem => eachItem.category === activeTabId)
    return newList
  }
  ...
  clickedImg = id => {
    const {score, listItems, randomImg} = this.state
    if (id === randomImg.id) {
      this.setState({
        score: score + 1,
        randomImg: listItems[Math.ceil(Math.random() * 30)],
      })
    }
  }
